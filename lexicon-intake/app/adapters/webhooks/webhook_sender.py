...
class WebhookSender:
    ...
    async def send_webhook(
        ...
        async with get_async_session_context() as db:
            ...
            delivery_record = delivery_records[0]  # Get first (should be only one)
            ...
            payload = WebhookPayload(
                ...
                reason_codes=lead_record.reason_codes,
            )
            ...
            for attempt in range(max_retries + 1):
                try:
                    logger.info(
                        ...
                        attempt=attempt + 1,
                        webhook_url=delivery_record.webhook_url,
                    )
                    
                    response = await self.client.post(
                        delivery_record.webhook_url,
